...
| ✅ **최종 결과물은 자체 제작만** | 개편이 끝난 페이지의 에셋은 직접 만든 **CSS · SVG · Canvas**만 커밋합니다 |
...
├── Mobilefirst/           # ┘
│
├── images/                # 참고용 외부 이미지·영상 (개편 1단계 제거·교체 대상)
├── *.ttf · *.otf          # 폰트 (라이선스 확인 후 정리)
└── (그 외 실습 잔재)
```
...
개편이 끝난 결과물의 에셋은 **직접 만든 것(CSS · SVG · Canvas)만** 커밋하는 것이 원칙입니다.
현재 repo에는 개편 전 실습 단계에서 **참고용으로 받아 둔 외부 이미지 · 영상 · 폰트가 아직 남아 있으며**, 개편 1단계에서 자체 제작 에셋으로 **교체 · 제거**합니다. (팬 제작 폰트 `Triforce.ttf` · `zelda.otf`는 라이선스 확인 후 정리 대상입니다.)
